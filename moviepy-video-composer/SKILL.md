...
- Use panel `treatment: "still_motion"` for subtle camera movement on a local/generated still. The composer applies the motion inside fullscreen, stack, and grid panel bounds. Use [scripts/render_still_motion.py](scripts/render_still_motion.py) only when a manually pre-rendered motion clip is explicitly needed.
- Use presenter panel `treatment: "overlay"` for fullscreen B-roll PiP.
- Use presenter panel `treatment: "blur"` plus `web-evidence` B-roll panel `treatment: "overlay"` for fullscreen evidence overlay. The composer uses the web-evidence PNG as the foreground proof card over a blurred presenter plate. This does not use LatentSync.
