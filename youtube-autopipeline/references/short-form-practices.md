...
- Use word-level generated spoken captions for reels; reserve `TEXT` entries and static labels for keyword beats, section turns, comparisons, or CTAs.
- Use visual changes when useful: A-roll, B-roll layouts, presenter panels, synthetic-motion, generated imagery, screen capture, or text overlays.
- Treat split-screen, two-stack, three-stack, four-grid, synthetic-motion captures, generated still motion, receipt highlights, kinetic text, before/after, myth/fact, and step-by-step/checklist sections as a non-ranked idea library. Choose by segment intent, not by fixed source priority.
- Enforce visual uniqueness by canonical source identity, not filename. Reused stock-provider ids, source URLs, copied local files, or repeated generated assets must be rejected unless reuse is intentional and recorded.
- For factual explainers, use verified visuals where the image itself supports the claim. Use generated visuals and generic stock as illustrative support, not as proof of exact species, product, person, place, or interface identity.
- When still images are the strongest source, pre-render motion clips from accepted stills with subtle push-ins, pans, swipes, or split-panel comparisons instead of filling the timeline with static frames.
- For reels over 45 seconds, aim for at least three accepted section patterns unless the brief explicitly calls for a constrained visual language.
- For modern reels, use word-level or phrase-level autogenerated captions from the approved transcript. Static `caption_text` overlays are labels, not spoken captions.
- Caption QA must inspect sync, clipping, contrast, line length, active-word highlighting, and safe-zone placement on the captioned final video.
- Treat frame QA as a required production stage. A completed MP4 is not deliverable until duration, resolution, captions, presenter overlays, text clipping, blank frames, visual variety, asset categories, and TTS prefix status are recorded.
...
- Synthetic-motion is the default source class for custom synthetic UI and abstract motion scenes, but board-created clips must be art-directed from a visual metaphor. Checklist, timeline, comparison, map, logistics, process, and dashboard are narrative intents, not reusable layouts. Use `z-image-turbo` when a generated still is the strongest photographic, cinematic, illustrative, or non-UI source for a segment; reject generic or placeholder-looking images.
- The composer must accept reviewed still images so generated visuals can be used directly as `B_ROLL` panel media.
...
- QA reports should include section-pattern variety, B-roll-backed source-type variety, repeated canonical IDs, and static-frame risk.
